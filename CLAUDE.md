...
- Run tests: `pytest` from repo root. Baseline: **421 passed, 3 skipped** (live tests skip without creds). Below baseline = regression.
...
5. **QA gate is fail-open** (flaky gate returns True so the generator still runs), but `QAConfigError` must surface — misconfiguration never hides behind fail-open.
...
Implemented: Haiku yes/no gate (~$0.0005/call, ~80-85% rejection) before
Sonnet Q+A (~$0.005/call) — 77% measured cut; `classify_answer` `latex` tier
keeps sympy-unparseable research math; QA generator disk-cached by statement
hash; `call_budget` enforced before every paid call.

Targets:
- **T2.1 Anthropic prompt caching** (biggest cheap win): neither `default_qa_quality_gate` nor `default_qa_generator` uses `cache_control`. Both send identical system prompts every call. Add `cache_control: {"type": "ephemeral"}` on system blocks; record `cache_read_input_tokens`.
- **T2.2 Gate result caching**: gate calls re-fire on resume (qa_cache.jsonl stores dict-or-None; bools would collide). Namespace keys (`gate:<sha1>` vs `qa:<sha1>`) or add second cache file in `ScrapeCheckpoint`. Keep torn-tail-tolerant `_iter_jsonl` semantics.
- **T2.3 Empirical planning ratios**: `PAPERS_PER_RECORD=4`, `CANDIDATES_PER_PAPER=3` are stale (live runs: 2-37 theorems/paper, ~13-20% gate survival vs the hardcoded 20% guess). Recalibrate from real run manifests; budgets derive from these.
- **T2.4 Gate batch mode** (investigate first): ~10 numbered theorems per Haiku call with JSON verdict array. Prototype behind opt-in flag; measure agreement vs single-call gate on ~50 cached theorems before defaulting. Fail-open per item.
- **T2.5 Live re-validation**: the two-stage gate has never completed a full live scrape (arXiv 429'd the pilots). First clean run: compare realized gate/qa calls + yield vs `_estimated_calls`, feed back into T2.3.

### Measurement discipline

- Baselines to beat: QA step $0.22/193 theorems; pipeline ~$0.32/25 records. Report before/after in the same units.
- Every paid call visible in manifests — no silent spend. New call type = new counter (`gate_calls` pattern) threaded through `ScrapeResult` → acquisition dict → `source_report.md` → `_estimated_calls`.
...
| concern | file |
|---|---|
| arXiv HTTP/pacing/backoff, two-stage QA | `src/icepick/allocation/scrape/realmath.py` |
...
Suggested order: T2.1 → T1.1 → T2.2 → T2.3 → T1.2/T1.3 (verify done) → T2.4 investigation → T2.5/T1.4 validation.

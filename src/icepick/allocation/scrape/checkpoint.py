...
class ScrapeCheckpoint:
    ...
    def caching_gate(self, gate: Callable) -> Callable:
        """Wrap the cheap QA quality gate with the same durable semantics.

        Gate calls are paid calls too. A cached ``False`` is as meaningful as
        a cached ``True``: both avoid re-spending Haiku on resume. Failures are
        not cached, preserving the scraper's fail-open behaviour for flaky
        gate calls.
        """
        ...
        def cached(statement, **kwargs):
            ...
            result = bool(gate(statement, **kwargs))
            with self._gate_cache_path.open("a", encoding="utf-8") as fh:
                fh.write(json.dumps({"key": key, "result": result}) + "\n")
                fh.flush()

...
CLI (run from ``src/loratrain`` with ``PYTHONPATH=src``)::
    ...
    # authorization @ 2abe292) -- see "v3b anchor_solved" below.
    python -m loratrain.v3 make-anchor-bundle --anchor-solutions <path> \\
        --bundle-dir <path>
    python -m loratrain.v3 build-dataset --bundle-dir <path> \\
        --solutions <path> --rollouts <path> --output-dir <path> \\
        --anchor-solutions <path> --anchor-rollouts <path>
...
v3b anchor_solved (2026-08-02, docs/SESSION_HANDOFF.md ledger authorization
@ 2abe292; PREREGISTRATION_V3.md Amendment 6 @ 0139327)
============================================================================

Stage 1 (the two subcommands above) trained on 390 proof-hinted rows drawn
from fail-heavy tiers and produced a null net effect plus solved-tier
degradation. v3b keeps those 390 hinted rows BYTE-UNCHANGED and ADDS
"anchor_solved" rows: proof-bearing, SOLVED-tier records (eval-excluded),
hint-regenerated by the IDENTICAL mechanism, so the model anchors on
problems it already solves. Two additive deltas, both gated so that with
no anchor input, behavior is byte-identical to pre-v3b code:

  1. New subcommand ``make-anchor-bundle`` -- mirrors ``make-regen-bundle``
     for an ``anchor_solutions.jsonl`` input (same hint-prompt
     construction, hint marker, terminal ``/no_think`` suffix, no-answer-
     key rule, atomic publish, idempotent-resume/``--force-new-dir``). The
     ONE substantive difference: ``assert_anchor_not_in_eval`` REPLACES
     ``assert_train_split_only`` (anchor_solved records are solved-tier
     and structurally ABSENT from ``train_side_uids`` -- excluded from the
     921-record proof-split universe at split-build time -- so the
     ordinary membership guard would hard-fail every single one as
     "unknown"; the actual proof needed is the OPPOSITE one, that the
     record is NOT on the eval side, at both uid and paper granularity).
  2. ``build-dataset`` gains optional, together-required
     ``--anchor-solutions`` / ``--anchor-rollouts`` (plus
     ``--hinted-reference-dataset``, defaulting to
     ``config.V3_STAGE1_HINTED_DATASET_PATH``): anchor rows are harvested
     by the SAME first-verified-wins chain as hinted rows, built via
     ``build_v3_sft_row`` with ``source_tier="anchor_solved"``, asserted
     cap1-disjoint from every hinted/legacy-anchor uid (never silently
     deduped), and appended to the final dataset -- NEVER counted in the
     60/40 collapse/band blend arithmetic and NEVER eligible for
     ``censuses.excluded_offtier`` (they are legitimately solved-tier).
     Before any of that, this run's freshly-built hinted rows are asserted
     byte-unchanged against the pinned stage-1 publish
     (``assert_hinted_subset_byte_unchanged`` -- 390 rows, sha16
     ``ad65a6e13a3e4e05``); any mismatch refuses the whole build.

NAME-COLLISION WARNING: "anchor_solved" is a COMPLETELY DIFFERENT
mechanism from the pre-existing "anchor" rows below (``draw_anchor_rows``,
``config.V3_ANCHOR_FRACTION`` -- RE-RULED to 0.0, Nicky's NO-ANCHOR
ruling, 2026-08-01 -- ``config.V3_ANCHOR_SEED_STRING``), which are
UNHINTED rows drawn deterministically from ``data/v2/cap1``. The two are
never combined, never share a uid pool, and ``V3_ANCHOR_FRACTION`` /
``draw_anchor_rows`` are untouched by this work.

``config.EVAL_SET_PATH``'s default (``evalharness/data/eval_set.jsonl``)
is STALE for the anchor_solved lane -- a 120-row file predating the
2026-08-01 split rebuild (verified by inspection: dated 2026-07-25,
carrying the OLD "eval_band"/"anchor_solved"/"anchor_fail" ``eval_slice``
vocabulary -- evalharness's OWN, UNRELATED use of the string
"anchor_solved" as a slice name, not this module's ``source_tier``). The
anchor-lane code never silently inherits that staleness: a resolved
``--eval-set`` file's row count is checked against
``config.V3_ANCHOR_EVAL_SET_EXPECTED_ROWS`` (286, the split's current
eval-set size) and a loud, NON-FATAL stderr warning fires on a mismatch
(``_warn_if_eval_set_row_count_unexpected``) -- never a hard refusal,
since this module cannot prove the caller's file is wrong, only that it
is unexpected.
"""
...
# v3b anchor_solved (docs/SESSION_HANDOFF.md ledger authorization @
# 2abe292) -- deliberately DIFFERENT filenames from the hinted bundle's,
# so a hinted bundle and an anchor bundle can coexist in the same
# --bundle-dir without collision (not that either subcommand requires
# that layout; it's just never made unsafe).
ANCHOR_BUNDLE_FILENAME = "anchor_regen_bundle.jsonl"
ANCHOR_BUNDLE_MANIFEST_FILENAME = "anchor_regen_bundle_manifest.json"

STAGE_REGEN_BUNDLE = "loratrain_v3_regen_bundle"
STAGE_BUILD_DATASET = "loratrain_v3_build_dataset"
STAGE_ANCHOR_BUNDLE = "loratrain_v3_anchor_bundle"
...
# v3b anchor_solved (docs/SESSION_HANDOFF.md ledger authorization @
# 2abe292): mirrors REGEN_BUNDLE_GUARD_STEPS with exactly one substantive
# substitution -- assert_anchor_not_in_eval REPLACES
# assert_train_split_only (never both; see assert_anchor_not_in_eval's
# docstring for why train-split membership is the wrong check for a
# solved-tier record).
ANCHOR_BUNDLE_GUARD_STEPS = (
    "assert_anchor_solutions_and_manifest_exist",
    "assert_anchor_solutions_sha_chain[generalized_chain_helper,not_forked]",
    "assert_manifest_split_pin",
    "assert_split_pinned[disk,reused=build_dataset.assert_split_pinned]",
    "load_anchor_solutions_rows",
    "assert_anchor_not_in_eval[uid_and_paper,SUBSTITUTES_assert_train_split_only--2abe292]",
    "assert_no_statement_leakage_exact[vs_eval_set.jsonl]",
    "build_regen_prompts[hint_marker+solution_text,suffix_terminal]",
    "assert_bundle_rows_well_formed[uid+regen_prompt_only,no_answer_key]",
    "resolve_publish_dir[restartability]",
...
# v3b anchor_solved (docs/SESSION_HANDOFF.md ledger authorization @
# 2abe292): additional guard steps ONLY run when --anchor-solutions is
# given (appended to BUILD_DATASET_GUARD_STEPS in the published manifest
# in that case; absent entirely otherwise -- additive, never a default
# behavior change).
BUILD_DATASET_ANCHOR_GUARD_STEPS = (
    "assert_hinted_subset_byte_unchanged[stage1_reference,ad65a6e1]",
    "assert_anchor_solutions_sha_chain[generalized_chain_helper,not_forked]",
    "assert_manifest_split_pin[anchor]",
    "assert_anchor_not_in_eval[uid_and_paper,SUBSTITUTES_assert_train_split_only--2abe292]",
    "assert_no_statement_leakage_exact[anchor_vs_eval_set.jsonl]",
    "load_anchor_rollouts",
    "select_first_verified_per_uid[anchor,ascending_sample_idx,cap_k_regen]",
    "assert_anchor_uids_disjoint_from_hinted_and_legacy_anchor[cap1,global,asserted_not_deduped]",
    "census_anchor_try_histogram_tier_table_hintcopy_attrition",
)
...
class EvalMembershipError(RuntimeError):
    """v3b anchor_solved (docs/SESSION_HANDOFF.md ledger authorization @
    2abe292): an anchor_solved candidate's uid or paper appears on the
    EVAL side of the pinned split (``eval_set_uids`` / ``papers.
    eval_papers``). This is the anchor lane's membership-guard
    counterpart of ``UnknownUidError`` -- anchor records are solved-tier
    and structurally ABSENT from ``train_side_uids`` (excluded from the
    921-record proof-split universe entirely at split-build time), so
    ``assert_train_split_only`` must never run against them (it would
    hard-fail every single one as "unknown", which is the wrong
    diagnosis). See ``assert_anchor_not_in_eval``.
    """


class AnchorInputError(RuntimeError):
    """v3b anchor_solved CLI/call-contract violation: ``--anchor-solutions``
    and ``--anchor-rollouts`` were not given together, or
    ``--hinted-reference-dataset`` is missing while an anchor input was
    given. Distinct from data-content errors (``SolutionsIntegrityError``,
    ``EvalMembershipError``, ...) -- this is about how the anchor inputs
    were INVOKED, not what they contain.
    """


class HintedReferenceMismatchError(RuntimeError):
    """v3b anchor_solved's one load-bearing invariant failed: the 390
    stage-1 hinted rows this run just built are not byte-unchanged
    relative to the pinned stage-1 reference dataset (docs/SESSION_
    HANDOFF.md ledger authorization @ 2abe292: "the 390 stage-1 hinted
    rows must come out byte-unchanged"). See
    ``assert_hinted_subset_byte_unchanged``.
    """
...
# v3b anchor_solved (docs/SESSION_HANDOFF.md ledger authorization @
# 2abe292): the SAME sha-chain shape as solutions_v3's (nested
# "<stem>.sha256" manifest key, flat "<stem>_sha256" key, the bare
# "sha256" key single-input manifests use, the input_shas.<stem> nesting
# the real proof-import publish uses; a stem-named sha sidecar beside the
# file itself) for the "anchor_solutions" stem -- generalized into
# ``assert_solutions_sha_chain`` via its ``manifest_sha_paths``/
# ``sidecar_names``/``label`` kwargs rather than forked into a second
# function.
ANCHOR_MANIFEST_SHA_PATHS = (
    "anchor_solutions.sha256", "anchor_solutions_sha256", "sha256", "input_shas.anchor_solutions",
)
ANCHOR_SHA_SIDECAR_NAMES = ("anchor_solutions.sha256", "anchor_solutions.jsonl.sha256")
...
def assert_solutions_sha_chain(
    solutions_path: Path,
    manifest_path: Path,
    manifest: dict,
    *,
    manifest_sha_paths: tuple = MANIFEST_SOLUTIONS_SHA_PATHS,
    sidecar_names: tuple = SOLUTIONS_SHA_SIDECAR_NAMES,
    label: str = "solutions",
) -> str:
    ...
    Generalized (not forked) for the v3b anchor_solved lane (docs/SESSION_
    HANDOFF.md ledger authorization @ 2abe292: "Generalize the existing
    chain helper"): the actual sha-comparison algorithm below is 100%
    shared code. ``make_anchor_bundle``/``build_dataset_cmd`` call this
    SAME function with ``manifest_sha_paths=ANCHOR_MANIFEST_SHA_PATHS,
    sidecar_names=ANCHOR_SHA_SIDECAR_NAMES, label="anchor solutions"``;
    every default parameter reproduces the pre-v3b solutions_v3 behavior
    byte-for-byte (including message text) when called with no kwargs.
    """
    ...
    if not solutions_path.exists():
        ...
        raise SolutionsIntegrityError(
            f"{solutions_path} not found -- the upstream mining lane has not "
            f"published its {label} file yet. This builder refuses to run "
            "without it."
        )
    recomputed = build_dataset.sha256_file(solutions_path)
    recorded = _lookup_first(manifest, *manifest_sha_paths)
    ...
    if recorded is None:
        raise SolutionsIntegrityError(
            f"{manifest_path}: no {label} sha recorded (looked for "
            + " / ".join(manifest_sha_paths)
            + ") and no sha sidecar ("
            + " / ".join(sidecar_names)
            + f") beside the {label} file -- refusing to trust an "
            f"unverifiable {label} file."
        )
    if not _recorded_sha_matches(recorded, recomputed):
        raise SolutionsIntegrityError(
            ...
            f">=16-hex prefix of the recomputed sha; the {label} file "
...
def load_split_eval_membership(split_path: Path, expected_split_sha256: str) -> tuple:
    """Pin-check the split (reuses ``build_dataset.assert_split_pinned``)
    then return ``(eval_uids: set, eval_papers: set)`` read from the split
    JSON's own ``eval_set_uids`` / ``papers.eval_papers`` keys.

    v3b anchor_solved's companion loader to ``load_split_uid_sets``
    (docs/SESSION_HANDOFF.md ledger authorization @ 2abe292): anchor
    records are solved-tier and therefore OUTSIDE the split's 921-record
    universe entirely (never members of ``train_side_uids`` -- solved-tier
    was excluded from that universe at split-build time, see
    split-rebuild-2026-08-01.md's ``excluded.solved_n``), so proving one
    is safe to anchor-train on means proving the OPPOSITE membership: that
    it does NOT sit on the eval side, by uid or by paper. ``eval_set_uids``
    is a dict keyed by tier in the real split file (band/collapse/
    misdirection) -- flattened here into one uid set, since anchor records
    don't carry a v3.py-relevant 60/40 tier of their own. Also tolerates a
    flat list shape for the same key (fixture-friendly).
    """
    split_path = Path(split_path)
    build_dataset.assert_split_pinned(split_path, expected_split_sha256)
    data = json.loads(split_path.read_text(encoding="utf-8"))
    raw_eval_uids = data.get("eval_set_uids") or {}
    if isinstance(raw_eval_uids, dict):
        eval_uids = {uid for uids in raw_eval_uids.values() for uid in uids}
    else:
        eval_uids = set(raw_eval_uids)
    eval_papers = set((data.get("papers") or {}).get("eval_papers") or [])
    return eval_uids, eval_papers


def assert_anchor_not_in_eval(rows, eval_uids: set, eval_papers: set, *, uid_key: str = "uid") -> None:
    """Hard-refuse if any anchor row's uid is in the split's
    ``eval_set_uids``, OR its ``provenance.arxiv_id`` is in the split's
    ``papers.eval_papers``. Collects every offender before raising (never
    fail-fast on the first one, same convention as
    ``assert_train_split_only``).

    v3b anchor_solved's membership-guard SUBSTITUTION for
    ``assert_train_split_only`` (docs/SESSION_HANDOFF.md ledger
    authorization @ 2abe292): anchor_solved records are solved-tier and
    therefore structurally ABSENT from ``train_side_uids`` -- running
    ``assert_train_split_only`` against them would hard-fail every single
    one as "unknown", which is the WRONG diagnosis. Absence from
    ``train_side_uids`` does not mean radioactive here, it means "not part
    of the 921-record proof-split universe at all" (solved-tier records
    were excluded from that universe entirely when the split was built).
    The invariant this module actually needs for an anchor candidate is
    the opposite one: proof that it is NOT on the EVAL side, checked at
    BOTH uid and paper granularity -- paper granularity because the
    split's own load-bearing guard is paper-level disjointness, not just
    uid-level (see config.py's ``V3_SPLIT_PATH`` comment).
    """
    # A row with no arxiv_id cannot be paper-checked at all. Refuse it rather
    # than let it slip past on a vacuous ``None in eval_papers`` -> False:
    # skipping half the substituted guard is exactly the failure this
    # substitution exists to avoid. (upload_guard's mirror of this rule
    # fail-closes the same way at the final gate; catching it here means the
    # refusal lands before any regen/training cost is spent.)
    paperless = sorted({
        r[uid_key] for r in rows
        if not str((r.get("provenance") or {}).get("arxiv_id") or "").strip()
    })
    if paperless:
        raise EvalMembershipError(
            f"{len(paperless)} anchor_solved record(s) carry no provenance.arxiv_id "
            f"({_shown(paperless)}) -- the paper-level eval-disjointness half of the "
            "anchor membership rule cannot run on them; refusing rather than "
            "passing them on a check that silently cannot fail."
        )
    uid_offenders = sorted({r[uid_key] for r in rows if r.get(uid_key) in eval_uids})
    paper_offenders = sorted({
        r[uid_key] for r in rows
        if (r.get("provenance") or {}).get("arxiv_id") in eval_papers
    })
    offenders = sorted(set(uid_offenders) | set(paper_offenders))
    if offenders:
        raise EvalMembershipError(
            f"{len(offenders)} anchor_solved record(s) are on the EVAL side of "
            f"the pinned split -- {len(uid_offenders)} by uid membership in "
            f"eval_set_uids ({_shown(uid_offenders)}), {len(paper_offenders)} "
            f"by paper membership in papers.eval_papers ({_shown(paper_offenders)}) "
            "-- an anchor_solved record must be provably NOT-in-eval at both "
            "granularities; refusing to proceed."
        )
...
def _assert_bundle_rows_well_formed(bundle_rows) -> None:
    """Defense in depth for freshly-built bundle rows: never trust
    construction alone (same idiom as build_dataset's "assert on
    examples, not just on the records that produced them"). Shared by
    ``make_anchor_bundle`` (v3b, docs/SESSION_HANDOFF.md ledger
    authorization @ 2abe292) -- ``make_regen_bundle`` keeps its own
    original inline copy of this exact check byte-unchanged, to avoid
    touching any code path the existing suite already exercises.
    """
...
# ============================================================================
# make-anchor-bundle (v3b anchor_solved -- docs/SESSION_HANDOFF.md ledger
...
def _warn_if_eval_set_row_count_unexpected(eval_set_path: Path) -> None:
    """Loud, NON-FATAL stderr warning when a resolved ``--eval-set`` file's
    row count doesn't match ``config.V3_ANCHOR_EVAL_SET_EXPECTED_ROWS``
    (286, the pinned split's current eval-set size).

    ``config.EVAL_SET_PATH``'s own default is a STALE 120-row file from
    before the 2026-08-01 split rebuild (verified by inspection: it is
    dated 2026-07-25 and its rows carry the OLD "eval_band"/"anchor_solved"
    /"anchor_fail" ``eval_slice`` vocabulary, not this split's band/
    collapse/misdirection tiers -- see config.py's comment beside
    ``V3_ANCHOR_EVAL_SET_EXPECTED_ROWS``). This function never hard-fails:
    it cannot prove the caller's file is WRONG, only that its row count is
    UNEXPECTED (a hermetic test fixture legitimately has neither count,
    and a legitimate future eval-set rebuild may legitimately change the
    number) -- but it refuses to let that staleness pass by SILENTLY.
    """
    n = _count_jsonl_rows(eval_set_path)
    if n != config.V3_ANCHOR_EVAL_SET_EXPECTED_ROWS:
        print(
            f"WARNING: --eval-set {eval_set_path} has {n} row(s), expected "
            f"{config.V3_ANCHOR_EVAL_SET_EXPECTED_ROWS} (the pinned split's "
            "current eval set) -- config.EVAL_SET_PATH's default is a STALE "
            "120-row pre-split-rebuild file; pass the correct --eval-set "
            "explicitly for the anchor_solved lane.",
            file=sys.stderr,
        )


def _anchor_bundle_input_signature(*, anchor_solutions_sha256, split_sha256, eval_set_sha256, k_regen) -> str:
    payload = {
        "anchor_solutions_sha256": anchor_solutions_sha256,
        "split_sha256": split_sha256,
        "eval_set_sha256": eval_set_sha256,
        "k_regen": k_regen,
        "hint_marker": config.V3_HINT_MARKER,
...
def make_anchor_bundle(
    *,
    ...
    """v3b anchor_solved counterpart of ``make_regen_bundle`` (docs/
    SESSION_HANDOFF.md ledger authorization @ 2abe292; PREREGISTRATION_
    V3.md Amendment 6 @ 0139327). Same hint prompt construction
    (``build_regen_prompt``), hint marker, terminal ``/no_think`` suffix,
    "no answer-key field in the bundle" rule, ``k_regen`` semantics, and
    atomic tmp->verify->rename publish + idempotent-resume/
    ``--force-new-dir`` sibling restartability as ``make_regen_bundle``.

    The ONE substantive difference is the membership guard: anchor_solved
    records are solved-tier and therefore OUTSIDE the split's 921-record
    train/eval universe (never members of ``train_side_uids``), so
    ``assert_train_split_only`` must NOT run against them -- see
    ``assert_anchor_not_in_eval``'s docstring for the full reasoning. The
    sha-chain check reuses ``assert_solutions_sha_chain`` unmodified via
    its generalization kwargs (``ANCHOR_MANIFEST_SHA_PATHS`` /
    ``ANCHOR_SHA_SIDECAR_NAMES``), never a forked copy.
    """
    anchor_solutions_path = Path(anchor_solutions_path)
    anchor_manifest_path = Path(anchor_manifest_path)
    split_path = Path(split_path)
    eval_set_path = Path(eval_set_path)
    bundle_dir = Path(bundle_dir)

    manifest = load_solutions_manifest(anchor_manifest_path)
    anchor_solutions_sha256 = assert_solutions_sha_chain(
        anchor_solutions_path, anchor_manifest_path, manifest,
        manifest_sha_paths=ANCHOR_MANIFEST_SHA_PATHS,
        sidecar_names=ANCHOR_SHA_SIDECAR_NAMES,
        label="anchor solutions",
    )
    assert_manifest_split_pin(manifest, anchor_manifest_path, expected_split_sha256[:16])

    # Membership SUBSTITUTION (docs/SESSION_HANDOFF.md ledger authorization
    # @ 2abe292): NOT assert_train_split_only -- see
    # assert_anchor_not_in_eval's docstring.
    eval_uids, eval_papers = load_split_eval_membership(split_path, expected_split_sha256)

    anchor_rows_in = load_solutions_rows(anchor_solutions_path)
    assert_anchor_not_in_eval(anchor_rows_in, eval_uids, eval_papers)

    if not eval_set_path.exists():
        raise SolutionsIntegrityError(f"{eval_set_path} not found -- cannot prove non-leakage")
    _warn_if_eval_set_row_count_unexpected(eval_set_path)
    eval_statements = build_dataset.load_eval_statements(eval_set_path)
    assert_no_statement_leakage_exact(anchor_rows_in, eval_statements)
    eval_set_sha256 = build_dataset.sha256_file(eval_set_path)

    bundle_rows = []
    for row in anchor_rows_in:
        regen_prompt = build_regen_prompt(row["question"], row["solution_text"])
        bundle_rows.append({"uid": row["uid"], "regen_prompt": regen_prompt})

    _assert_bundle_rows_well_formed(bundle_rows)

    content_bytes = _jsonl_bytes(bundle_rows)
    bundle_sha256 = _sha256_bytes(content_bytes)

    input_signature = _anchor_bundle_input_signature(
        anchor_solutions_sha256=anchor_solutions_sha256,
        split_sha256=expected_split_sha256,
        eval_set_sha256=eval_set_sha256,
        k_regen=k_regen,
    ...
    publish_dir, existing = _resolve_publish_dir(
        bundle_dir,
        force_new_dir=force_new_dir,
        manifest_name=ANCHOR_BUNDLE_MANIFEST_FILENAME,
        expected_stage=STAGE_ANCHOR_BUNDLE,
        input_signature=input_signature,
    )
    if existing is not None:
        return existing
    ...
    publish_dir.mkdir(parents=True, exist_ok=True)
    bundle_path = publish_dir / ANCHOR_BUNDLE_FILENAME
    bundle_tmp_path = publish_dir / (ANCHOR_BUNDLE_FILENAME + ".tmp")
    manifest_out_path = publish_dir / ANCHOR_BUNDLE_MANIFEST_FILENAME
    ...
    if len(reread_rows) != len(bundle_rows):
        raise BundleIntegrityError(
            f"post-write verification read {len(reread_rows)} row(s), expected "
            f"{len(bundle_rows)} -- anchor bundle NOT published (tmp left for forensics)."
        )
    for brow in reread_rows:
        if any(k not in brow for k in REQUIRED_BUNDLE_ROW_KEYS) or "answer" in brow or "solution_text" in brow:
            raise BundleIntegrityError(
                f"post-write verification found a malformed row: {brow} -- "
                "anchor bundle NOT published (tmp left for forensics)."
            )
    bundle_tmp_path.replace(bundle_path)

    out_manifest = {
        "stage": STAGE_ANCHOR_BUNDLE,
        "lane": "anchor_solved",
        "created": datetime.now(timezone.utc).isoformat(),
        "anchor_solutions": {
            "path": str(anchor_solutions_path), "sha256": anchor_solutions_sha256, "rows": len(anchor_rows_in),
        },
        "anchor_solutions_manifest_path": str(anchor_manifest_path),
        "split": {
            "path": str(split_path),
            "sha256": expected_split_sha256,
            "membership_guard_note": (
                "GUARD SUBSTITUTION (docs/SESSION_HANDOFF.md ledger authorization "
                "@ 2abe292): assert_train_split_only is NOT applied to "
                "anchor_solved records -- they are solved-tier and structurally "
                "absent from train_side_uids (excluded from the 921-record "
                "proof-split universe at split-build time). Instead this bundle "
                "asserts NOT-in-eval: uid not in split.eval_set_uids AND paper "
                "not in split.papers.eval_papers (assert_anchor_not_in_eval)."
            ),
        },
        "eval_set": {
            "path": str(eval_set_path),
            "sha256": eval_set_sha256,
            "rows": _count_jsonl_rows(eval_set_path),
            "freshness_note": (
                f"expected {config.V3_ANCHOR_EVAL_SET_EXPECTED_ROWS} rows (the "
                "split's current eval set) -- config.EVAL_SET_PATH's default is "
                "a STALE 120-row pre-split-rebuild file; a row-count mismatch is "
                "a stderr WARNING only (see config.V3_ANCHOR_EVAL_SET_EXPECTED_"
                "ROWS), not a hard refusal -- this module cannot know the "
                "caller's file is wrong, only that it is unexpected."
            ),
        },
        "k_regen": k_regen,
        "generation_params": {
            ...
            "system_prompt": config.PASS_AT_K_SYSTEM_PROMPT,
            "no_think_suffix": config.PASS_AT_K_NO_THINK_SUFFIX,
            "backend_note": "qwen_http on a box (P2) -- out of scope for this module; identical generation params to the hinted lane.",
        },
        "hint_marker": config.V3_HINT_MARKER,
        "bundle": {"path": str(bundle_path), "sha256": bundle_sha256, "rows": len(bundle_rows)},
        "published_files": {ANCHOR_BUNDLE_FILENAME: bundle_sha256},
        "input_signature": input_signature,
        "guards": list(ANCHOR_BUNDLE_GUARD_STEPS),
        "resumed_from_existing_publish": False,
    }
    manifest_out_path.write_text(json.dumps(out_manifest, indent=2), encoding="utf-8")
    return out_manifest
...
# ============================================================================
# v3b anchor_solved helpers (docs/SESSION_HANDOFF.md ledger authorization @
# 2abe292; PREREGISTRATION_V3.md Amendment 6 @ 0139327).
#
# NAME-COLLISION WARNING: "anchor_solved" here is a COMPLETELY DIFFERENT
# mechanism from ``draw_anchor_rows``/``V3_ANCHOR_FRACTION``/
# ``V3_ANCHOR_SEED_STRING`` immediately above -- that legacy "anchor" is an
# UNHINTED v2/cap1 row drawn deterministically to fill out the R3 blend
# (currently retired to a 0.0 fraction, Nicky's NO-ANCHOR ruling,
# 2026-08-01). "anchor_solved" is a HINTED, proof-bearing, solved-tier
# record built by the SAME hint-regeneration mechanism as the 390 stage-1
# hinted rows, added on TOP of them, never touching V3_ANCHOR_FRACTION or
# draw_anchor_rows in any way. The two are never combined, never share a
# uid pool, and are never referred to interchangeably below.
# ============================================================================


def _word_ngrams(text: str, n: int = 6) -> set:
    words = (text or "").split()
    if len(words) < n:
        return set()
...
def hint_copy_fraction(hint_text: str, completion_text: str, n: int = 6) -> float:
    """Fraction of the hint's word-``n``-grams (default 6) that also
    appear among the kept completion's word-``n``-grams -- the EXACT
    method the stage-1 regen census used (confirmed by inspection,
    ``out/v3_full_run_20260801/sft_dataset/hint_copy_census.json``:
    ``"note": "fraction of hint word-6-grams appearing in kept
    completion"``). A hint shorter than ``n`` words has no n-grams to
    check at all -- vacuously ``0.0`` (no evidence of copying), not an
    error.
    """
    hint_grams = _word_ngrams(hint_text, n)
    if not hint_grams:
        return 0.0
    completion_grams = _word_ngrams(completion_text, n)
    return len(hint_grams & completion_grams) / len(hint_grams)


def assert_hinted_subset_byte_unchanged(hinted_rows: list, reference_dataset_path: Path) -> None:
    """Hard-refuse unless ``hinted_rows`` (this run's freshly-built hinted
    rows, BEFORE any anchor_solved rows are appended) is content-identical
    to the pinned stage-1 published dataset -- v3b's one load-bearing
    invariant (docs/SESSION_HANDOFF.md ledger authorization @ 2abe292:
    "the 390 stage-1 hinted rows must come out byte-unchanged").

    Compared by uid (never by list position): both sides are parsed JSON
    dicts, and this run's bundle/solutions-file iteration order is not
    something this invariant should depend on. "Byte-unchanged" is
    verified via ``json.dumps(row, sort_keys=True)`` equality per uid --
    strictly STRONGER than a raw-bytes diff of the two files (immune to
    incidental key-order/whitespace differences that would be false
    positives for "changed", while still catching any real difference in
    content), which is why this is a dict-equality check rather than a
    literal file-bytes comparison. Judgment call, recorded here.
    """
    reference_dataset_path = Path(reference_dataset_path)
    if not reference_dataset_path.exists():
        raise HintedReferenceMismatchError(
            f"{reference_dataset_path} not found -- cannot prove the stage-1 "
            "hinted rows are byte-unchanged (v3b's one load-bearing invariant, "
            "docs/SESSION_HANDOFF.md ledger authorization @ 2abe292); refusing "
            "to proceed without the pinned comparison target."
        )
    reference_rows = []
    with reference_dataset_path.open("r", encoding="utf-8") as fh:
        for line in fh:
            line = line.strip()
            if line:
                reference_rows.append(json.loads(line))

    # Soft, NON-FATAL early-warning canary against config.
    # V3_STAGE1_HINTED_DATASET_EXPECTED_{SHA16,ROWS} -- deliberately a
    # warning, not a hard gate: the REAL guarantee is the full per-row
    # comparison below, which must stay hermetically testable against a
    # small fixture reference file that will never match the real stage-1
    # file's row count/sha16. This canary exists only to give a fast,
    # loud signal in production if --hinted-reference-dataset was pointed
    # at the wrong file altogether.
    reference_sha16 = build_dataset.sha256_file(reference_dataset_path)[:16]
    if (
        reference_sha16 != config.V3_STAGE1_HINTED_DATASET_EXPECTED_SHA16
        or len(reference_rows) != config.V3_STAGE1_HINTED_DATASET_EXPECTED_ROWS
    ):
        print(
            f"WARNING: --hinted-reference-dataset {reference_dataset_path} has "
            f"{len(reference_rows)} row(s), sha16={reference_sha16} -- expected "
            f"{config.V3_STAGE1_HINTED_DATASET_EXPECTED_ROWS} row(s), sha16="
            f"{config.V3_STAGE1_HINTED_DATASET_EXPECTED_SHA16} (the pinned "
            "stage-1 publish). Proceeding with the real per-row comparison "
            "regardless -- this is an early-warning canary, not a hard gate.",
            file=sys.stderr,
        )
    ...
    for row in reference_rows:
        ...
        if uid in ref_by_uid:
            raise HintedReferenceMismatchError(
                f"{reference_dataset_path}: duplicate uid {uid!r} in the stage-1 "
                "reference dataset -- malformed reference, refusing."
            )
        ref_by_uid[uid] = row
    hinted_by_uid = {_row_uid(row): row for row in hinted_rows}

    if set(ref_by_uid) != set(hinted_by_uid):
        only_ref = sorted(set(ref_by_uid) - set(hinted_by_uid))
        only_hinted = sorted(set(hinted_by_uid) - set(ref_by_uid))
        raise HintedReferenceMismatchError(
            "hinted-row uid set differs from the stage-1 reference dataset "
            f"({reference_dataset_path}): {len(only_ref)} uid(s) only in the "
            f"reference: {_shown(only_ref)}; {len(only_hinted)} uid(s) only in "
            f"this run's hinted rows: {_shown(only_hinted)}. The stage-1 hinted "
            "rows must come out byte-unchanged; refusing to proceed."
        )
    mismatched = sorted(
        uid for uid, ref_row in ref_by_uid.items()
        if json.dumps(ref_row, sort_keys=True) != json.dumps(hinted_by_uid[uid], sort_keys=True)
    )
    if mismatched:
        raise HintedReferenceMismatchError(
            f"{len(mismatched)} hinted row(s) differ in content from the stage-1 "
            f"reference dataset ({reference_dataset_path}): {_shown(mismatched)} "
            "-- the stage-1 hinted rows must come out byte-unchanged; refusing "
            "to proceed."
        )
...
def _dataset_input_signature(
    *,
    ...
    v2_cap1_sha256,
    k_regen,
    anchor_solutions_sha256=None,
    anchor_rollouts_sha256=None,
    hinted_reference_sha256=None,
) -> str:
    payload = {
        "solutions_sha256": solutions_sha256,
        ...
        # v3b anchor_solved (2abe292) -- None/None/None whenever
        # --anchor-solutions is not given, so a no-anchor build's signature
        # still changes value here relative to pre-v3b code (an inevitable,
        # harmless consequence of widening the payload's key-space) but its
        # EQUALITY/INEQUALITY behavior across repeated no-anchor builds is
        # unaffected -- restart/resume idempotency is a relative check, not
        # a check against a hardcoded signature string (see test_v3.py).
        "anchor_solutions_sha256": anchor_solutions_sha256,
        "anchor_rollouts_sha256": anchor_rollouts_sha256,
        "hinted_reference_dataset_sha256": hinted_reference_sha256,
    }
    return _sha256_bytes(json.dumps(payload, sort_keys=True).encode("utf-8"))
...
def build_dataset_cmd(
    *,
    ...
    verify_fn=None,
    force_new_dir: bool = False,
    anchor_solutions_path: Path = None,
    anchor_manifest_path: Path = None,
    anchor_rollouts_path: Path = None,
    hinted_reference_dataset_path: Path = None,
) -> dict:
    ...
    v3b anchor_solved (docs/SESSION_HANDOFF.md ledger authorization @
    2abe292; PREREGISTRATION_V3.md Amendment 6 @ 0139327), ADDITIVE ONLY:
    ``anchor_solutions_path`` and ``anchor_rollouts_path`` are optional and
    REQUIRED TOGETHER (``AnchorInputError`` if only one is given). When
    both are ``None`` (the default), this function's behavior -- including
    the exact bytes of the published dataset and the exact shape of the
    published manifest -- is IDENTICAL to pre-v3b code; every anchor-only
    code path below is gated on ``anchor_solutions_path is not None``. When
    given, ``hinted_reference_dataset_path`` is also required (the pinned
    stage-1 dataset this run's hinted rows must prove byte-unchanged
    against -- see ``assert_hinted_subset_byte_unchanged``).
    """
    bundle_dir = Path(bundle_dir)
    solutions_path = Path(solutions_path)
    manifest_path = Path(manifest_path)
    ...
    if (anchor_solutions_path is None) != (anchor_rollouts_path is None):
        raise AnchorInputError(
            "--anchor-solutions and --anchor-rollouts must be given together "
            f"(got anchor_solutions_path={anchor_solutions_path!r}, "
            f"anchor_rollouts_path={anchor_rollouts_path!r})."
        )
    if anchor_solutions_path is not None and hinted_reference_dataset_path is None:
        raise AnchorInputError(
            "--hinted-reference-dataset is required whenever --anchor-solutions "
            "is given -- the stage-1 hinted rows must be asserted byte-unchanged "
            "(docs/SESSION_HANDOFF.md ledger authorization @ 2abe292); refusing "
            "to proceed without a pinned comparison target."
        )
    if anchor_solutions_path is not None:
        anchor_solutions_path = Path(anchor_solutions_path)
        anchor_rollouts_path = Path(anchor_rollouts_path)
        anchor_manifest_path = (
            Path(anchor_manifest_path) if anchor_manifest_path is not None
            else anchor_solutions_path.parent / "manifest.json"
        )
        hinted_reference_dataset_path = Path(hinted_reference_dataset_path)
    ...
    # Captured BEFORE any anchor_solved rows are appended below -- the
    # blend block's own "final_rows" must report the R3 (hinted + legacy
    # v2/cap1 anchor) blend's subtotal only. anchor_solved is a SEPARATE
    # mechanism that is never counted in this arithmetic (module docstring
    # "cap1" / docs/SESSION_HANDOFF.md ledger authorization @ 2abe292);
    # the TRUE final published total (including anchor_solved) is reported
    # at manifest["dataset"]["rows"] instead.
    r3_blend_final_rows = len(final_rows)

    # ========================================================================
    # v3b anchor_solved (docs/SESSION_HANDOFF.md ledger authorization @
    # 2abe292; PREREGISTRATION_V3.md Amendment 6 @ 0139327). ADDITIVE ONLY --
    # this entire block (and every variable it touches below) is gated on
    # anchor_solutions_path is not None; with no --anchor-solutions, nothing
    # here executes and the published dataset bytes / manifest shape are
    # IDENTICAL to pre-v3b code.
    # ========================================================================
    anchor_census = None
    if anchor_solutions_path is not None:
        # This is v3b's one load-bearing invariant: the hinted rows this
        # run just built (BEFORE the legacy v2/cap1 anchor or anchor_solved
        # rows are appended) must be byte-unchanged relative to the stage-1
        # publish. Checked FIRST, before spending any effort on the anchor
        # harvest, so a stage-1 drift is refused as early as possible.
        assert_hinted_subset_byte_unchanged(hinted_rows, hinted_reference_dataset_path)
        hinted_reference_sha256 = build_dataset.sha256_file(hinted_reference_dataset_path)

        # --- guards: anchor solutions / split-membership chain (defense in
        # depth, full re-verification -- build-dataset is the ONLY place
        # that verifies these for the anchor lane; make-anchor-bundle is not
        # a required upstream step here, unlike the hinted bundle) ---------
        anchor_manifest = load_solutions_manifest(anchor_manifest_path)
        anchor_solutions_sha256 = assert_solutions_sha_chain(
            anchor_solutions_path, anchor_manifest_path, anchor_manifest,
            manifest_sha_paths=ANCHOR_MANIFEST_SHA_PATHS,
            sidecar_names=ANCHOR_SHA_SIDECAR_NAMES,
            label="anchor solutions",
        )
        assert_manifest_split_pin(anchor_manifest, anchor_manifest_path, expected_split_sha256[:16])

        # Membership SUBSTITUTION (2abe292): NOT assert_train_split_only --
        # see assert_anchor_not_in_eval's docstring for why.
        eval_uids, eval_papers = load_split_eval_membership(split_path, expected_split_sha256)
        anchor_solutions_rows = load_solutions_rows(anchor_solutions_path)
        assert_anchor_not_in_eval(anchor_solutions_rows, eval_uids, eval_papers)
        assert_no_statement_leakage_exact(anchor_solutions_rows, eval_statements)
        _warn_if_eval_set_row_count_unexpected(eval_set_path)

        anchor_rollouts_by_uid, anchor_rollouts_census = load_rollouts(anchor_rollouts_path)
        anchor_rollouts_sha256 = build_dataset.sha256_file(anchor_rollouts_path)

        # --- per-uid first-verified-wins harvest (SAME chain as hinted rows;
        # NO tier-resolution gating -- anchor_solved rows are legitimately
        # solved-tier and must NEVER land in excluded_offtier) -------------
        anchor_solved_rows = []
        anchor_try_histogram: dict = {}
        anchor_hint_insufficient_uids = []
        anchor_missing_from_rollouts_uids = []
        anchor_tier_census: dict = {}
        anchor_hint_copy_per_uid: dict = {}
        for srow in anchor_solutions_rows:
            uid = srow["uid"]
            samples = anchor_rollouts_by_uid.get(uid)
            if not samples:
                anchor_missing_from_rollouts_uids.append(uid)
                continue
            found = select_first_verified(samples, srow["answer"], k_regen, verify_fn)
            if found is None:
                anchor_hint_insufficient_uids.append(uid)
                continue
            arxiv_id = (srow.get("provenance") or {}).get("arxiv_id")
            anchor_solved_rows.append(
                build_v3_sft_row(
                    uid=uid,
                    question=srow["question"],
                    output=found["output"],
                    ...
                    sample_idx=found["sample_idx"],
                    k_tried=found["k_tried"],
                    source_tier="anchor_solved",
                    arxiv_id=arxiv_id,
                )
            )
            anchor_try_histogram[found["k_tried"]] = anchor_try_histogram.get(found["k_tried"], 0) + 1
            # Informational only (never gates inclusion/exclusion): what tier
            # this solved-tier candidate WOULD have resolved to under the
            # hinted-lane's resolver, for the manifest's "tier table".
            _bucket, resolved_label = resolve_hinted_tier(uid, band_corpus_uids, wellposed_pool_by_uid)
            anchor_tier_census[resolved_label] = anchor_tier_census.get(resolved_label, 0) + 1
            anchor_hint_copy_per_uid[uid] = round(hint_copy_fraction(srow["solution_text"], found["output"]), 4)

        # cap1, GLOBAL (module docstring "cap1"): anchor_solved uids must be
        # disjoint from every uid already destined for the final dataset
        # (hinted rows AND any legacy v2/cap1 anchor rows) -- asserted, never
        # silently deduped.
        already_used_uids = {_row_uid(r) for r in final_rows}
        anchor_solved_uids = {_row_uid(r) for r in anchor_solved_rows}
        collision = already_used_uids & anchor_solved_uids
        if collision:
            raise BlendError(
                f"{len(collision)} anchor_solved uid(s) collide with an "
                f"already-kept hinted/anchor row: {_shown(sorted(collision))} -- "
                "cap1 requires disjoint uids across the WHOLE final dataset "
                "(module docstring 'cap1'); refusing to silently dedupe."
            )

        final_rows = final_rows + anchor_solved_rows

        anchor_census = {
            "lane": "anchor_solved",
            "note": (
                "v3b anchor_solved rows (docs/SESSION_HANDOFF.md ledger "
                "authorization @ 2abe292; PREREGISTRATION_V3.md Amendment 6 @ "
                "0139327): proof-bearing, solved-tier, eval-excluded records, "
                "hint-regenerated by the IDENTICAL mechanism as the hinted "
                "rows, cap1-disjoint from every hinted/anchor uid. NEVER "
                "counted in the blend block's 60/40 collapse/band arithmetic "
                "and NEVER eligible for censuses.excluded_offtier -- they are "
                "legitimately solved-tier, not off-tier. COMPLETELY DIFFERENT "
                "mechanism from the legacy 'anchor' rows in the blend block "
                "above (draw_anchor_rows / V3_ANCHOR_FRACTION) -- never "
                "combined, never sharing a uid pool."
            ),
            "membership_guard_note": (
                "GUARD SUBSTITUTION: assert_train_split_only is NOT applied to "
                "anchor_solved records (they are structurally absent from "
                "train_side_uids -- solved-tier was excluded from the 921-"
                "record proof-split universe at split-build time). Instead: "
                "uid not in split.eval_set_uids AND paper not in "
                "split.papers.eval_papers (assert_anchor_not_in_eval)."
            ),
            "inputs": {
                "anchor_solutions": {
                    "path": str(anchor_solutions_path), "sha256": anchor_solutions_sha256,
                    "rows": len(anchor_solutions_rows),
                },
                "anchor_solutions_manifest_path": str(anchor_manifest_path),
                "anchor_rollouts": {
                    "path": str(anchor_rollouts_path), "sha256": anchor_rollouts_sha256, **anchor_rollouts_census,
                },
                "hinted_reference_dataset": {
                    "path": str(hinted_reference_dataset_path), "sha256": hinted_reference_sha256,
                    "rows": len(hinted_rows),
                },
            },
            "count": len(anchor_solved_rows),
            "fraction_of_final_dataset": (len(anchor_solved_rows) / len(final_rows)) if final_rows else None,
            "try_histogram": {str(k): v for k, v in sorted(anchor_try_histogram.items())},
            "tier_table": {str(k): v for k, v in sorted(anchor_tier_census.items())},
            "attrition": {
                "missing_from_rollouts": {
                    "count": len(anchor_missing_from_rollouts_uids), "uids": sorted(anchor_missing_from_rollouts_uids),
                },
                "hint_insufficient": {
                    "count": len(anchor_hint_insufficient_uids), "uids": sorted(anchor_hint_insufficient_uids),
                },
            },
            "hint_copy_census": {
                "per_uid": anchor_hint_copy_per_uid,
                "note": "fraction of hint word-6-grams appearing in kept completion",
            },
            "hinted_byte_identity_check": {
                "reference_path": str(hinted_reference_dataset_path),
                "reference_sha256": hinted_reference_sha256,
                "reference_rows": len(hinted_rows),
    ...
    input_signature = _dataset_input_signature(
        ...
        k_regen=k_regen,
        anchor_solutions_sha256=(anchor_census["inputs"]["anchor_solutions"]["sha256"] if anchor_census else None),
        anchor_rollouts_sha256=(anchor_census["inputs"]["anchor_rollouts"]["sha256"] if anchor_census else None),
        hinted_reference_sha256=(anchor_census["inputs"]["hinted_reference_dataset"]["sha256"] if anchor_census else None),
    )
    ...
    out_manifest = {
        ...
        "blend": {
            ...
            "anchor_seed_string": config.V3_ANCHOR_SEED_STRING,
            "final_rows": r3_blend_final_rows,
        },
        ...
        "input_signature": input_signature,
        "guards": list(BUILD_DATASET_GUARD_STEPS) + (list(BUILD_DATASET_ANCHOR_GUARD_STEPS) if anchor_census else []),
        "resumed_from_existing_publish": False,
    }
    if anchor_census is not None:
        out_manifest["anchor_solved"] = anchor_census
    manifest_out_path.write_text(json.dumps(out_manifest, indent=2), encoding="utf-8")
    return out_manifest
...
def validate_v3_config() -> None:
    ...
    if (
        not isinstance(config.V3_ANCHOR_EVAL_SET_EXPECTED_ROWS, int)
        or isinstance(config.V3_ANCHOR_EVAL_SET_EXPECTED_ROWS, bool)
        or config.V3_ANCHOR_EVAL_SET_EXPECTED_ROWS < 1
    ):
        problems.append(
            "V3_ANCHOR_EVAL_SET_EXPECTED_ROWS must be a positive int "
            f"(got {config.V3_ANCHOR_EVAL_SET_EXPECTED_ROWS!r})"
        )
    if not _hex16(config.V3_STAGE1_HINTED_DATASET_EXPECTED_SHA16):
        problems.append(
            "V3_STAGE1_HINTED_DATASET_EXPECTED_SHA16 must be 16 hex chars "
            f"(got {config.V3_STAGE1_HINTED_DATASET_EXPECTED_SHA16!r})"
        )
    if (
        not isinstance(config.V3_STAGE1_HINTED_DATASET_EXPECTED_ROWS, int)
        or isinstance(config.V3_STAGE1_HINTED_DATASET_EXPECTED_ROWS, bool)
        or config.V3_STAGE1_HINTED_DATASET_EXPECTED_ROWS < 1
    ):
        problems.append(
            "V3_STAGE1_HINTED_DATASET_EXPECTED_ROWS must be a positive int "
            f"(got {config.V3_STAGE1_HINTED_DATASET_EXPECTED_ROWS!r})"
        )
    if problems:
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        ...
        description=(
            "v3 proof-as-hint regeneration dataset builder (skeleton P1, plus "
            "the v3b anchor_solved lane). Three subcommands: make-regen-bundle, "
            "make-anchor-bundle, build-dataset. No flag here can override a sha "
            "pin or skip/weaken a guard; --force-new-dir only resolves an "
            "output-directory collision (see module docstring)."
        ),
    )
    ...
    a = sub.add_parser(
        "make-anchor-bundle",
        help=(
            "v3b: build the hint-augmented regen prompt bundle for anchor_solved "
            "records (solved-tier, proof-bearing, NOT-in-eval -- a COMPLETELY "
            "DIFFERENT mechanism from the legacy, currently-0.0-fraction "
            "'anchor' rows build-dataset draws from v2/cap1; see module "
            "docstring's anchor_solved section)."
        ),
    )
    a.add_argument(
        "--anchor-solutions", type=Path, required=True,
        help="Path to anchor_solutions.jsonl (solved-tier, proof-bearing, NOT-in-eval records). Required, explicit -- no default, no auto-discovery.",
    )
    a.add_argument(
        "--anchor-manifest", type=Path, default=None,
        help="Path to the anchor solutions file's manifest.json (default: its sibling manifest.json).",
    )
    a.add_argument("--split", type=Path, default=config.V3_SPLIT_PATH, help="Path to the pinned split file.")
    a.add_argument(
        "--eval-set", type=Path, default=config.EVAL_SET_PATH,
        help=(
            "Path to eval_set.jsonl. WARNING: config.EVAL_SET_PATH's default is "
            "a STALE 120-row pre-split-rebuild file (see config."
            "V3_ANCHOR_EVAL_SET_EXPECTED_ROWS) -- pass the current 286-record "
            "eval set explicitly. A row-count mismatch is warned loudly at "
            "runtime (stderr) but not refused."
        ),
    )
    a.add_argument(
        "--bundle-dir", type=Path, required=True,
        help="Output dir for anchor_regen_bundle.jsonl + its manifest. Required -- no default.",
    )
    a.add_argument(
        "--force-new-dir", action="store_true",
        help="If --bundle-dir already holds a completed publish from DIFFERENT inputs, publish under a fresh sibling dir instead of refusing (the existing dir is left untouched).",
    ...
    d.add_argument(
        "--anchor-solutions", type=Path, default=None,
        help=(
            "v3b: path to anchor_solutions.jsonl (solved-tier, proof-bearing, "
            "NOT-in-eval records). Optional -- ADDITIVE: with no --anchor-"
            "solutions, output is byte-identical to pre-v3b behavior. Required "
            "together with --anchor-rollouts."
        ),
    )
    d.add_argument(
        "--anchor-manifest", type=Path, default=None,
        help="v3b: path to the anchor solutions file's manifest.json (default: its sibling manifest.json). Only meaningful with --anchor-solutions.",
    )
    d.add_argument(
        "--anchor-rollouts", type=Path, default=None,
        help="v3b: anchor box regen outputs, same {uid, sample_idx, output} shape as --rollouts. Required together with --anchor-solutions.",
    )
    d.add_argument(
        "--hinted-reference-dataset", type=Path, default=config.V3_STAGE1_HINTED_DATASET_PATH,
        help=(
            "v3b: the pinned stage-1 published dataset (390 rows, sha16 "
            f"{config.V3_STAGE1_HINTED_DATASET_EXPECTED_SHA16}) this run's "
            "hinted rows must be byte-unchanged against. Required whenever "
            "--anchor-solutions is given; ignored otherwise."
        ),
    )
...
def main(argv=None) -> int:
    ...
    if args.subcommand == "make-regen-bundle":
    ...
    elif args.subcommand == "make-anchor-bundle":
        anchor_manifest_path = args.anchor_manifest or (args.anchor_solutions.parent / "manifest.json")
        manifest = make_anchor_bundle(
            anchor_solutions_path=args.anchor_solutions,
        ...
        assert args.subcommand == "build-dataset", f"unknown subcommand {args.subcommand!r}"
        anchor_manifest_path = None
        if args.anchor_solutions is not None:
            anchor_manifest_path = args.anchor_manifest or (args.anchor_solutions.parent / "manifest.json")
        manifest = build_dataset_cmd(
            ...
            anchor_solutions_path=args.anchor_solutions,
            anchor_manifest_path=anchor_manifest_path,
            anchor_rollouts_path=args.anchor_rollouts,
            hinted_reference_dataset_path=(
                args.hinted_reference_dataset if args.anchor_solutions is not None else None
            ),
        )
        summary = {
            "stage": manifest["stage"],
        ...
        if "anchor_solved" in manifest:
            summary["anchor_solved"] = {
                k: manifest["anchor_solved"][k] for k in ("count", "fraction_of_final_dataset")
            }
